...
| Valid file format | a) File size is less then 5 mb. b) Valid documen witht future expiry date. c) Select only one file while uploading | Upload file in pdf or JPG format. | Positve | Automated UI and API | UI should show the uploaded document. API should return 202 accepted. |
...
| Replace existing uploaded file | a) Valid File format JPG or PDF. b) Valid file size less then 5 mb. c) Valid document with future expiry date. | Upload a new file to replace the existing file. | Positve | Automated UI | UI should show the new uploaded document in file preview and the old file should not be visible. |
| Upload the same file again | a) Valid File format JPG or PDF. b) Valid file size less then 5 mb. c) Valid document with future expiry date. | Upload same  file to replace the existing file. | Positve | Automated UI | UI should show the only one uploaded document in file preview. |
| Upload file with file name including special characters | a) Valid File format JPG or PDF. b) Valid file size less then 5 mb. c) Valid document with future expiry date. d) Select only one file while uploading. e) File name containing special characters | Upload file with file name including special characters i.e ()"$#* e.t.c. | Positve | Automated UI | UI should show the uploaded file. |
...
| Valid Until date of document of today | a) Valid File format JPG or PDF. b) File size equal to 5 mb. c) Valid document with expiry date of today. d) Select only one file while uploading. | Upload file with valid unti date of todays date. | Negative Edge case | Manual UI test | Valid uploaded file is rejected after review or processing. |
...
Hi team,  
I have found a bug in the recent feature that behaves differently from the product specifications or actal expected behavior. While using the system as an Invester to upload a valid file, The user is able to successfully upload file on the frontend web application and sees success message of upload. However, Due to backend job or the API layer error the file does not get saved into the backend. This shows that the UI and backend is not synched for proper success and error handling. The impact of this is that user will have the confirmation that the file is uploaded but in reality it is not in the system. This would cause user to upload file again and again with a success message everytime. It will also cause waste of resourced due to frequent API calls and processing.

**Improvement Suggestion:**  
a) I would suggest to add snyching flow of UI and API in the E2E testing suite or if budget constraints are present then focus on Integration testing.
